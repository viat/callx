...
void logConfig() {
    ...
    L_i
            << "socket_output_remote_port: "
            << callxConfig->socket_output_remote_port;
    L_i
            << "socket_output_send_seconds: "
    ...
    L_i
            << "sba_call_completion_attemps_min: "
...
void run(CallxConfig *callxConfig) {
    ...
    for (int i = 1; i <= callxConfig->tp_repository_size; i++) {
        ...
        tlayerPacketQueue->push(move(make_unique<TlayerPacket,
                TlayerPacketRecycler>()));
    }
    L_t
            << "The TlayerPacket repository has got "
    ...
    if (callxConfig->start_console) {
        L_i
                << "Console has been created but will not be started, because start_console is false.";
        console->start();
    }
    pcapHandler->start();
    ...
    L_i
            << "Stop requested. Stopping threads now.";
    ...
    if (console_stopped && pcapHandler_stopped && tlayerDispatcher_stopped
        ...
        L_t
                << "All threads are stopped. Calling join().";
        console->join();
        pcapHandler->join();
    ...
    L_i
            << "Cleaning up and deleting shared resources.";
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        L_t
                << "Caught an unknown exception in run().";
    }
