...
bool SipProcessor::stop() {
    L_t
            << "Overridden stop() has been called, deactivating SipPacketQueue.";
...
void SipProcessor::worker() {
    ...
    while (!m_stopRequested) {
        ...
        if (!m_sipPacketQueue->waitAndPop(m_currPacket)) {
            ...
            L_t
                    << "wait_and_pop() results false -> SipPacketQueue has been "
                    << "deactivated.";
            L_t
                    << "m_stopRequested: "
                    << m_stopRequested;
        ...
        if (!m_currPacket->parse()) {
            L_i
                    << "Parsing SIP packet failed, discarding packet.";
            continue;
        }
        ...
        if (m_currCall->findTransaction( { m_currPacket->getBranch(),
                ...
                || (m_currPacket->getCseqMethod() == rm_ACK
                        && m_currCall->findTransaction( { m_currPacket
                                ->getBranch(), rm_INVITE },
                                m_currTransaction))) {
...
void SipProcessor::handleCancel() {
    ...
    if (!m_currCall->findTransaction(inviteTransactionId, inviteTransaction)) {
        ...
        L_t
                << "Transaction error: CANCEL request does not match an "
    ...
    L_t
            << "Found the INVITE transaction to be canceled.";

    // check SentBy
    if (m_currTransaction->getSentBy() != inviteTransaction->getSentBy()) {
        // SentBy does not match
        L_t
                << "SentBy value of the CANCEL request does not match the one in "
    ...
    if (inviteTransaction->getCseqNum() != m_currTransaction->getCseqNum()) {
        L_t
                << "CSeq Number in CANCEL transaction does not match the "
                << "INVITE transaction. Call ID: "
                << m_currPacket->getCallId();
    }
...
void SipProcessor::handleAckOnNonSuccess() {
    ...
    if (!matchState( { { ts_TERMINATED, ts_COMPLETED } })) {
        L_t
                << "Transaction error. State is not (client / server): "
...
void SipProcessor::handleAckOnSuccess() {
    ...
    if (!m_currCall->findInviteTransaction(inviteTransaction)) {
        ...
        L_t
                << "Transaction error: ACK request does not match an INVITE "
                << "transaction. Call ID: "
                << m_currPacket->getCallId();
        return;
    }
    ...
    if (m_currTransaction->getSentBy() != inviteTransaction->getSentBy()) {
        // SentBy does not match
        L_t
                << "SentBy of the ACK request does not match the one in "
                << "the initial INVITE request. Call ID: "
                << m_currPacket->getCallId();
        return;
    }
    ...
    if (inviteTransaction->getCseqNum() != m_currTransaction->getCseqNum()) {
        L_t
                << "CSeq Number in ACK transaction does not match the "
                << "INVITE transaction. Call ID: "
                << m_currPacket->getCallId();
        return;
    }
    ...
    // found INVITE transaction
    L_t
            << "Found the INVITE transaction to this ACK request.";
...
void SipProcessor::processSdp() {
    ...
    L_t
            << "Processing Session Description.";
    ...
    if (m_currCall->getReinviteFlag()) {
        if (m_rtpSinkMap->find(rtpSocket, rtpSink)) {
            if (rtpSink->getStatus() == rs_CONFIRMED) {
                ...
                L_t
                        << "RtpSink does exist. Changing state from CONFIRMED to CONFIRMED_INIT.";
            } else {
                L_i
    ...
    if (!m_currCall->rtpSinksAreActivated()) {
        L_t
                << "Deactivate RtpSink because Call object flag"
    ...
    L_t
            << "Inserting RtpSink Object into local and into global RtpSinkMap.";
    m_rtpSinkMap->insert( { rtpSocket, rtpSink });
    L_t
            << "Size of local RtpSinkMap is now: "
...
void SipProcessor::deleteCallFromCallMap(const string& callId) {
    ...
    if (count == 0) {
        L_e
                << "Deleting Call reference from CallMap failed. "
...
void SipProcessor::prepareCallToDecode(const string& callId) {
    ...
    m_callDecodeQueue->push(m_currCall);
}
