...
SigBasedAna::SigBasedAna() :
		m_callxConfig(CallxConfig::getInstance()), m_sbaEventMap(
				SbaEventMap::getInstance()), m_sbaIncidentMap(
				SbaIncidentMap::getInstance()) {
	L_t<< "C'tor";
...
void SigBasedAna::worker() {
	...
	while (!m_stopRequested) {
		...
		L_t<< "Signaling-based Analyzer starting.";
		analyze();
		L_t
		...
		while ((m_workerCond.wait_until(workerCondLock, wakeupTime) == std::cv_status::timeout)
				&& !m_stopRequested) {
		}
	}
	L_t
...
void SigBasedAna::analyzeCallAttempts() {
	...
	if (call_attempts > call_attempts_max) {
		...
		insertIncident(m_currCaller,
				make_shared<SbaIncident>(it_CALL_ATTEMPTS, m_currCaller, period,
						call_attempts, true, reasonStream.str()));

		string type = incidentTypeToString(it_CALL_ATTEMPTS);

		L_t<< incidentTypeToString(it_CALL_ATTEMPTS)
		<< ": "
		<< reasonStream.str();
...
void SigBasedAna::analyzeCallsConcurrent() {
	...
	if (calls_concurrent > calls_concurrent_max) {
		...
		insertIncident(m_currCaller,
				make_shared<SbaIncident>(it_CALLS_CONCURRENT, m_currCaller,
						period, calls_concurrent, true, reasonStream.str()));
		L_t<< incidentTypeToString(it_CALLS_CONCURRENT)
		<< ": "
		<< reasonStream.str();
	}
}
...
void SigBasedAna::analyzeCallCompletion() {
	...
	int completion_ratio = floor(
			((calls_completed * 1.0 / call_attemps) * 100) + 0.5);
	...
	if (completion_ratio < call_completion_min) {
		...
		insertIncident(m_currCaller,
				make_shared<SbaIncident>(it_CALL_COMPLETION, m_currCaller,
						period, completion_ratio, false, reasonStream.str()));
		L_t<< incidentTypeToString(it_CALL_COMPLETION)
		<< ": "
		<< reasonStream.str();
	}
...
void SigBasedAna::analyzeCallDurationAverage() {
	...
	duration_average = floor(
			((duration_cumulative * 1.0 / call_counter) / 1000) + 0.5);
	...
	if (duration_average < duration_average_min) {
		...
		insertIncident(m_currCaller,
				make_shared<SbaIncident>(it_CALL_DURATION_AVERAGE, m_currCaller,
						period, duration_average, false, reasonStream.str()));
		L_t<< incidentTypeToString(it_CALL_DURATION_AVERAGE)
		<< ": "
		<< reasonStream.str();
	}
}
...
void SigBasedAna::analyzeCallDurationCumulative() {
	...
	int duration_cumulative_max =
			m_callxConfig->sba_call_duration_cumulative_max;
	int duration_cumulative = 0;
	...
	if (duration_cumulative_minutes > duration_cumulative_max) {
		...
		insertIncident(m_currCaller,
				make_shared<SbaIncident>(it_CALL_DURATION_CUMULATIVE,
						m_currCaller, period, duration_cumulative_minutes, true,
						reasonStream.str()));
		L_t<< incidentTypeToString(it_CALL_DURATION_CUMULATIVE)
		<< ": "
		<< reasonStream.str();
	}
}
...
void SigBasedAna::analyzeCallsClosedByCallee() {
	...
	callee_closed_ratio = floor(
			(closed_by_callee * 1.0 / call_counter * 100) + 0.5);
	...
	if (call_counter >= min_completed
		...
		insertIncident(m_currCaller,
				make_shared<SbaIncident>(it_CALLS_CLOSED_BY_CALLEE,
						m_currCaller, period, callee_closed_ratio, true,
						reasonStream.str()));
		L_t<< incidentTypeToString(it_CALLS_CLOSED_BY_CALLEE)
		<< ": "
		<< reasonStream.str();
	}
}
...
bool SigBasedAna::isInPeriod(systemClock::time_point eventTs, int period) {
	return (boost::chrono::duration_cast<minutes>(m_analyzeTs - eventTs).count()
			< period
			&& boost::chrono::duration_cast<seconds>(m_analyzeTs - eventTs).count()
					> 0);
}
