...
Call::Call(string callId) :
		m_creationTs(systemClock::now()), m_creationTsSteady(
				steadyClock::now()), m_activityTs(steadyClock::now()), m_localRtpSinkMap(
				make_shared<RtpSinkMapType>()), m_rtpSinkMap(
				RtpSinkMap::getInstance()), m_dialog(
				make_shared<Dialog>(callId)), m_reinviteFlag(false), m_rtpSinksAreActivated(
				true) {
	L_t<< "C'tor";
}
...
void Call::checkRecordingTime(int maxSeconds) {
	...
	if (m_rtpSinksAreActivated && diff.count() > maxSeconds) {
		for (auto sinkIter = m_localRtpSinkMap->begin();
			...
			L_t<< "RtpSink deactivated after " << diff.count() << " seconds, socket: "
			<< sinkIter->first;
		}
...
void Call::unregisterRtpSinks() const {
	int count = 0;
	L_t<< "Deleting RtpSink references from global RtpSinkMap.";
	...
	L_t<< "Deleted " << count << " RtpSink references.";
}
